...
4. 60초 timeout 후 자동으로 에러 메시지 표시됨
5. 문제가 계속되면 잠시 후 다시 시도

> 💡 **참고**: API 호출은 최대 60초까지 대기하며, 시간 초과 시 명확한 에러 메시지가 표시됩니다.
...
### 에러: "Gemini API 호출 시간 초과 (60초)"

**원인**: API 응답이 60초 이내에 도착하지 않았습니다.

**해결 방법**:
1. 네트워크 연결이 안정적인지 확인
2. 이미지 파일 크기가 너무 크지 않은지 확인 (권장: 5MB 이하)
3. 잠시 후 다시 시도
4. VPN 사용 시 VPN을 끄고 시도
5. 문제가 계속되면 Google AI Studio에서 API 상태 확인
